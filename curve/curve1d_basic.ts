...
class EquationCurve extends CurveTypeData {
  ...
  makeGUI(
    container: Record<string, Function>,
    ...
    let row = (container.row as Function)() as Record<string, Function>;

    let text = ((this.uidata as Record<string, unknown>).textbox = (row.textbox as Function)(
      undefined,
      "" + this.equation
    ));
    (text as Record<string, unknown>).onchange = (val: string) => {
      console.log(val);
      this.equation = val;
      this.update();
      this.redraw();
    };
    (container.label as Function)("Equation");
  }

  killGUI(_container: Record<string, Function>): void {
    if (this.uidata !== undefined) {
      let ud = this.uidata as Record<string, unknown>;
      if (ud.textbox) {
        (ud.textbox as { remove(): void }).remove();
      }
    }
...
class GuassianCurve extends CurveTypeData {
  ...
  equals(b: CurveTypeData): boolean {
    ...
    let gb = b as unknown as GuassianCurve;
  ...
  makeGUI(
    ...
    let ud = this.uidata as Record<string, unknown>;
    ud.hslider = (container.slider as Function)(undefined, "Height", this.height, -10, 10, 0.0001);
    (ud.hslider as Record<string, unknown>).onchange = () => {
      this.height = ((this.uidata as Record<string, unknown>).hslider as { value: number }).value;
      this.redraw();
      this.update();
    };
    ud.oslider = (container.slider as Function)(undefined, "Offset", this.offset, -10, 10, 0.0001);
    (ud.oslider as Record<string, unknown>).onchange = () => {
      this.offset = ((this.uidata as Record<string, unknown>).oslider as { value: number }).value;
      this.redraw();
      this.update();
    };
    ud.dslider = (container.slider as Function)(undefined, "STD Deviation", this.deviation, -10, 10, 0.0001);
    (ud.dslider as Record<string, unknown>).onchange = () => {
      this.deviation = ((this.uidata as Record<string, unknown>).dslider as { value: number }).value;
      this.redraw();
      this.update();
    };
  }
  ...
  killGUI(_container: Record<string, Function>): void {
    if (this.uidata !== undefined) {
      let ud = this.uidata as Record<string, unknown>;
      (ud.hslider as { remove(): void }).remove();
      (ud.oslider as { remove(): void }).remove();
      (ud.dslider as { remove(): void }).remove();
    }
