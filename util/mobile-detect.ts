// @ts-nocheck -- Third-party generated file (mobile-detect v1.4.4)
...
interface MobileDetectConstructor {
  new (userAgent: string, maxPhoneWidth?: number): MobileDetectInstance;
  (this: MobileDetectInstance, userAgent: string, maxPhoneWidth?: number): void;
  prototype: MobileDetectInstance;
  isPhoneSized(maxPhoneWidth: number): boolean | undefined;
  _impl: MobileDetectImpl;
  version: string;
}
...
function prepareUserAgent(userAgent: string): string {
  ...
  for (key in mobileDetectRules.props) {
    if (hasOwnProp.call(mobileDetectRules.props, key)) {
      ...
      if (!isArray(rawValues)) {
        ...
        values = rawValues as (string | RegExp)[];
      }
      len = values.length;
      for (i = 0; i < len; ++i) {
      ...
      mobileDetectRules.props[key] = values as RegExp[];
    }
  }
  convertPropsToRegExp(mobileDetectRules.oss as Record<string, string | RegExp>);
  convertPropsToRegExp(mobileDetectRules.phones as Record<string, string | RegExp>);
  convertPropsToRegExp(mobileDetectRules.tablets as Record<string, string | RegExp>);
  convertPropsToRegExp(mobileDetectRules.uas as Record<string, string | RegExp>);
  convertPropsToRegExp(mobileDetectRules.utils as Record<string, string | RegExp>);

  // copy some patterns to oss0 which are tested first (see issue#15)
  mobileDetectRules.oss0 = {
    WindowsPhoneOS : mobileDetectRules.oss.WindowsPhoneOS as RegExp,
    WindowsMobileOS: mobileDetectRules.oss.WindowsMobileOS as RegExp,
  };
})();
 ...
 * @private
 */
impl.findMatch = function (rules: Record<string, string | RegExp>, userAgent: string): string | null {
 ...
 * @global
 */
function MobileDetect(this: MobileDetectInstance, userAgent: string, maxPhoneWidth?: number) {
...
MobileDetect.prototype = {
   ...
   */
  versionStr: function (key: string) {
   ...
   */
  match: function (pattern: string | RegExp) {
